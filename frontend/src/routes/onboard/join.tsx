...
export const Route = createFileRoute("/onboard/join")({
  component: JoinPage,
});
...
function JoinPage() {
  ...
  React.useEffect(() => {
    async function validateToken() {
      ...
      try {
        ...
        if (data.invitation.type !== "student") {
          setValidationError("This invitation is not for a student");
          setLoading(false);
          return;
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const resp = await fetchWithAuth("invitations/complete-student-join", {
        method: "POST",
        body: JSON.stringify({
          token,
          password,
      ...
      <div className="max-w-xl mx-auto">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Create your account to start earning badges
            </CardDescription>
          </CardHeader>
          <CardContent>
            {error && (
